        ...
        <tr>
            <asp:Repeater ID="Repeater2" runat="server">           
              <ItemTemplate>
                <td>
                 <center>
                 <asp:HyperLink ID="HyperLink1" runat="server" Text ='<%# Bind("Link_English_Name") %>' NavigateUrl='<%# Bind("Link_URL")%>'  >HyperLink</asp:HyperLink>
                </center></td> 
              </ItemTemplate>
            </asp:Repeater>
        </tr>
...
<asp:GridView ID="links_grid" SkinID="links_grid" CssClass="Links_style" runat="server" AutoGenerateColumns="False"
    ShowHeader="true" ShowFooter="true"><%-- Width="100%"--%>
    <Columns>
        <asp:TemplateField HeaderText="Related Links">
            ...
            <table>
                ...
                <td><asp:HyperLink ID="HyperLink1" CssClass="LinksItems" runat="server" Text='<%# Bind("Link_English_Name") %>'
            ...
            <asp:Image CssClass="LinksImage"  id = "Image" runat= "server" ImageUrl =  '<%# String.Concat(ImagePath() ,Eval("Link_Logo_Path"))%>' Height = "35px" Width = "35px" Visible ='<%#System.IO.File.Exists(Server.MapPath (string.Concat(ImagePath(), Eval("Link_Logo_Path")))) %>' />
                ...
                <td>
                <asp:HyperLink ID="HyperLink1" CssClass="LinksItems" runat="server" Text='<%# Bind("Link_English_Name") %>'
                    NavigateUrl='<%# Bind("Link_URL")%>'>HyperLink</asp:HyperLink>
            </td>
            </tr>
            </table>
            </AlternatingItemTemplate>
            <HeaderStyle CssClass="LinkGridH" />
        </asp:TemplateField>
    </Columns>
